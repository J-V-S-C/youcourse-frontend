...
import {
  ExpandMore,
  PlayCircleOutlined as PlayCircleOutline,
  Lock,
...
import { formatPrice } from '@/app/components/courses/courseUtils';
    ...
    return (
      ...
      >
        ...
        <Container sx={{ py: 10, textAlign: 'center', flexGrow: 1 }}>
          <Typography variant="h4" color="var(--foreground)">
    ...
    >
      ...
      >
        <Container maxWidth="lg">
          <Typography
            variant="h3"
            sx={{ fontWeight: 'bold', color: 'var(--foreground)', mb: 2 }}
          >
            {course.name}
          </Typography>
          <Typography
            variant="h6"
            sx={{ color: 'var(--muted)', mb: 4, maxWidth: '800px' }}
          >
            {course.description}
          </Typography>
          <Button
          ...
          >
            {course.sellable && course.price
              ? `Comprar por ${formatPrice(course.price)}`
              : 'Comece a aprender agora (Grátis)'}
          </Button>
        </Container>
      </Box>
      ...
      <Box sx={{ flexGrow: 1, py: 8 }}>
        <Container maxWidth="md">
          <Typography
            variant="h5"
            sx={{ fontWeight: 'bold', color: 'var(--foreground)', mb: 4 }}
          >
            Conteúdo do Curso
          </Typography>
          ...
          {unitsWithLessons.length === 0 ? (
            ...
            unitsWithLessons
              ...
              .map((unit) => (
                ...
                >
                  <AccordionSummary
                    expandIcon={<ExpandMore sx={{ color: 'var(--muted)' }} />}
                  >
                    <Box>
                      <Typography
                        variant="subtitle1"
                  ...
                  <AccordionDetails sx={{ p: 0 }}>
                    ...
                    <List disablePadding>
                      {unit.lessons
                        ...
                        .map((lesson, idx) => {
                          const isClickable =
                            lesson.isPreview && lesson.video?.playbackUrl;
                      ...
                      {unit.lessons.length === 0 && (
                        <ListItem sx={{ px: 3, py: 2 }}>
                          <ListItemText
                            primary="Nenhuma aula divulgada."
                            sx={{ color: 'var(--muted)' }}
                          />
                        </ListItem>
                      )}
